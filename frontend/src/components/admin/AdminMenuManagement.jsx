...
const AdminMenuManagement = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [menuRes, restRes] = await Promise.all([
        api.get(`/admin/menu?user_id=${userId}`),
        api.get('/restaurants')
      ]);
  ...
  const groupedItems = items.reduce((group, item) => {
    if (!group[item.restaurantId]) {
      group[item.restaurantId] = [];
    }
    group[item.restaurantId].push(item);
    return group;
  }, {});
  ...
  return (
    <div className="space-y-8">
      {Object.keys(groupedItems).map(restaurantId => {
        const title = restaurants[restaurantId]?.name || `Restaurant #${restaurantId}`;
        return (
          <div key={restaurantId} className="space-y-4">
            <div className="flex items-center gap-2 mb-4 border-b border-border pb-2 text-foreground">
              <Store className="w-5 h-5 text-primary" />
              <h3 className="text-xl font-bold">{title}</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {groupedItems[restaurantId].map(item => (
