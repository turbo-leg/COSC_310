import { ArrowLeft, Clock, CreditCard, Loader2, Minus, Navigation, Plus, ShoppingBag } from 'lucide-react';
...
export default function OrderCheckout() {
  ...
  const applyPromoCode = async () => {
    ...
    try {
      ...
      if (cleanedPromo) payload.promo_code = cleanedPromo;

      console.log("Applying promo with payload:", payload); // debug

      const res = await api.post('/orders/calculate-total-cost', payload);
      setTotalCost(res.data.total_order_cost);
      setAppliedPromo(cleanedPromo);
    } catch (err) {
  ...
  if (!restaurant) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-muted text-muted-foreground p-6">
        ...
        <button onClick={() => navigate('/')} className="bg-primary text-primary-foreground px-6 py-2 rounded-full font-bold">
          Back to Home
        </button>
      </div>
    );
    ...
    <div className="min-h-screen bg-muted text-muted-foreground py-10 px-4 md:px-12 animate-in fade-in duration-500">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-card text-card-foreground rounded-[2rem] shadow-sm border border-border overflow-hidden flex flex-col md:flex-row">
          ...
          {/* Cart Section */}
          <div className="md:w-1/3 bg-primary text-primary-foreground p-8 flex flex-col relative overflow-hidden">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-primary text-primary-foreground text-sm px-4 py-2.5 rounded-lg border border-border focus:border-primary-500 focus:ring-1 focus:ring-primary-500 outline-none"
                        />
                      </div>
                      <div>
            ...
            {appliedPromo && (
              <p className="text-green-500 mt-1 text-sm">Promo "{appliedPromo}" applied!</p>
            )}
