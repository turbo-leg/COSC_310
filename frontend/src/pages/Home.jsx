...
import { Store, ArrowRight, Utensils, AlertCircle, Search, SearchX, ShoppingBag, ChevronDown, ChevronUp } from 'lucide-react';
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {restaurants.map(r => (
              <div 
                key={r.userId} 
                className={`bg-white rounded-3xl shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-500 overflow-hidden ${expandedId === r.userId ? 'ring-2 ring-blue-500/20' : ''}`}
              >
                <div 
                ...
                >
                  ...
                  <div className="mt-4 flex items-center justify-between pt-4 border-t border-gray-50">
                    <div 
                      ...
                      onClick={(e) => { e.stopPropagation(); toggleExpand(r.userId); }}
                    >
                      {expandedId === r.userId ? 'Hide Menu' : 'View Menu'}
                      {expandedId === r.userId ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </div>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/restaurant/${r.userId}`);
                      }}
                ...
                <div 
                  className={`overflow-hidden transition-all duration-500 ease-in-out bg-gray-50 ${
                    expandedId === r.userId ? 'max-h-screen opacity-100 border-t border-gray-100' : 'max-h-0 opacity-0'
                  }`}
                >
                  <div className="p-6">
                    ...
                    {!menus[r.userId] ? (
                      ...
                      <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                        {menus[r.userId].map(item => (
                          <div key={item.item_id} className="group/item flex items-center justify-between p-4 bg-white rounded-2xl border border-gray-100 hover:border-blue-200 hover:shadow-md transition-all">
                            <div>
