...
def test_admin_toggle_menu_item_stock():
    ...
    # 2. Test regular user cannot toggle
    payload = {"isActive": False}
    response = client.put(f"/admin/menu/{item_id}/toggle-stock?user_id={regular_user['userId']}", json=payload)
    assert response.status_code == 403
    
    # 3. Test admin can toggle to out of stock
    response = client.put(f"/admin/menu/{item_id}/toggle-stock?user_id={admin_user['userId']}", json=payload)
    ...
    # 4. Test admin can toggle back to in stock
    payload_true = {"isActive": True}
    response = client.put(f"/admin/menu/{item_id}/toggle-stock?user_id={admin_user['userId']}", json=payload_true)
    assert response.status_code == 200
...
def test_admin_toggle_nonexistent_item():
    ...
    response = client.put(f"/admin/menu/9999/toggle-stock?user_id={admin_user['userId']}", json={"isActive": False})
